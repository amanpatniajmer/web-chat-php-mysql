...
switch ($_POST['submit']) {
    case 'login':
        ...
        if ($result = mysqli_query($db, $query)) {
            if ($row = mysqli_fetch_assoc($result)) {
                $a=rand(1000000000,1000000000000);
                setcookie('authorize', $a, time() + 60 * 60, '/; samesite=strict');
                $b="UPDATE users SET id=".$a." WHERE username='".$_POST['username']."'";
                $c=mysqli_query($db,$b);
    ...
    case 'check':
        $b="SELECT id FROM users WHERE username='".$_POST['username']."'";
        $c=mysqli_query($db,$b);
        if ($row = mysqli_fetch_assoc($c)) {
            if ($_COOKIE['authorize']!=$row['id']) {
                echo "Invalid User";
            }
            else{
                $status=$_POST['status'];
        ...
        break;
    case 'logout':
        setcookie('authorize', -10, time() - 60 * 60, '/; samesite=strict');
        $username=$_POST['username'];
